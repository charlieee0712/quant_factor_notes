...
author: 量化拯救散户
date: '2026-05-19'
fetched_at: '2026-06-22T02:31:38'
title: 筹码分布因子，IC不高，但是分层结果相当出色！
url: https://mp.weixin.qq.com/s?__biz=MzkwNjYzMTEyMg==&mid=2247496768&idx=1&sn=8f1818c558b626bd6d89ffacb6b8d73f
---

![](https://mmbiz.qpic.cn/sz_mmbiz_gif/cvcTnuxE6ibZNjC9TcJo8mBjcvMlZ4xj5u7IcZ6zicNu1GpdibUutaWBWkdVduNsN9cYrnzVcI35EtVooOM8YKQWLF1h8WocoFJA5SLI6bXHP8/640?wx_fmt=gif&from=appmsg)
...
![](images/2026-05-19_筹码分布因子，IC不高，但是分层结果相当出色！/001.png)
...
![](images/2026-05-19_筹码分布因子，IC不高，但是分层结果相当出色！/002.png)

![](images/2026-05-19_筹码分布因子，IC不高，但是分层结果相当出色！/003.png)
...
首先，读取换手率和复权因子的数据。在笔者的数据中，是后复权因子。同时，还用一个字典last\_chip来保存上一时刻各个标的的筹码分布。

后复权，就是保持开始的时候的价格不变，改变后面的价格。

前复权，则是保持当前价格不变，改变之前的价格。

理解了这个原理，就很容易将后复权转换为前复权了。后面的代码将会对此详细说明。
...
这段代码主要是读取数据，然后调用cal\_factors方法，为每个标的计算因子。感觉并不需要太多的介绍了。
...
第6-7行，没有上一个交易日的筹码数据时的计算。

第8-13行，有上一个交易日的筹码数据时的计算。这里需要注意的是第9行，通过除以当前时刻的后复权因子，将价格转换成了前复权的价格。（所以也就第27-28行，对价格乘以当前的复权因子，再次变成后复权的价格）

第二块，14行之后，计算因子值。

14-16行，其实是错了的，应该直接获取最低价和最高价就行了。笔者想复杂了。

第18-20行，是一个用来计算加权偏度、峰度和标准差的函数。
...
![](images/2026-05-19_筹码分布因子，IC不高，但是分层结果相当出色！/004.png)
...
![](images/2026-05-19_筹码分布因子，IC不高，但是分层结果相当出色！/005.png)

![](images/2026-05-19_筹码分布因子，IC不高，但是分层结果相当出色！/006.png)

![](images/2026-05-19_筹码分布因子，IC不高，但是分层结果相当出色！/007.png)

![](images/2026-05-19_筹码分布因子，IC不高，但是分层结果相当出色！/008.png)

![](images/2026-05-19_筹码分布因子，IC不高，但是分层结果相当出色！/009.png)

![](images/2026-05-19_筹码分布因子，IC不高，但是分层结果相当出色！/010.png)
...
![](images/2026-05-19_筹码分布因子，IC不高，但是分层结果相当出色！/011.png)

![](images/2026-05-19_筹码分布因子，IC不高，但是分层结果相当出色！/012.png)

**03**

**换手率分析**

![](images/2026-05-19_筹码分布因子，IC不高，但是分层结果相当出色！/013.png)

![](images/2026-05-19_筹码分布因子，IC不高，但是分层结果相当出色！/014.png)

**04**

**收益分析**

![](images/2026-05-19_筹码分布因子，IC不高，但是分层结果相当出色！/015.png)

![](images/2026-05-19_筹码分布因子，IC不高，但是分层结果相当出色！/016.png)

这个分层回测看起来还行。在整个回测区间里，其年化收益率的单调性是完美的，因子值越小的组年化收益率越高。

至于用标准差低频化后的分层回测结果，其因子值最大的一组遥遥落后其他四组，而其他四组可以说是你中有我我中有你，无法区分。

![](images/2026-05-19_筹码分布因子，IC不高，但是分层结果相当出色！/017.png)
...
![](https://mmbiz.qpic.cn/mmbiz_gif/cvcTnuxE6ibZmW1QaCWnR3oZDpt26eJXRC9BQ14mDOrw1soV6ib6ZAkCvvLW4zic1nLUe2GyRibsYxM5DrlCSgCW9oWfcQX87M9chmviauf4afz0/640?wx_fmt=gif&from=appmsg)