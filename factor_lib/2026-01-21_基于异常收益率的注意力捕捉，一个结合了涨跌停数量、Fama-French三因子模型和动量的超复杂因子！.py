"""基于异常收益率的注意力捕捉，一个结合了涨跌停数量、Fama-French三因子模型和动量的超复杂因子！

自动生成自 ../sources/chenshengrui/articles/2026-01-21_基于异常收益率的注意力捕捉，一个结合了涨跌停数量、Fama-French三因子模型和动量的超复杂因子！.md。
所有 # 注释内容均直接摘录自原文，未经改写或归纳。
图片位于 ../sources/chenshengrui/articles/images/2026-01-21_基于异常收益率的注意力捕捉，一个结合了涨跌停数量、Fama-French三因子模型和动量的超复杂因子！/
"""
...
# 来源标识: chenshengrui
# 公众号  : 量化拯救散户
# 标题    : 基于异常收益率的注意力捕捉，一个结合了涨跌停数量、Fama-French三因子模型和动量的超复杂因子！
# 日期    : 2026-01-21
# 原文 URL: https://mp.weixin.qq.com/s?__biz=MzkwNjYzMTEyMg==&mid=2247494289&idx=1&sn=ac78d20e2d7ae806dd70c3ecea64ed73
# 本地原文: ../sources/chenshengrui/articles/2026-01-21_基于异常收益率的注意力捕捉，一个结合了涨跌停数量、Fama-French三因子模型和动量的超复杂因子！.md
# 本地图片: ../sources/chenshengrui/articles/images/2026-01-21_基于异常收益率的注意力捕捉，一个结合了涨跌停数量、Fama-French三因子模型和动量的超复杂因子！/  (共 16 张)
...
# (作者注) 第2-5行，读取行业数据，并确定开始时间和标的数量。这样，后面读取行情和估值数据就能直接对齐了。
#
# (作者注) 第6-8行，读取行情数据，这次除了读取收盘价和复权因子，还读取了涨停价和跌停价。
#
# (作者注) 第9-10行，计算全市场涨跌停标的的占比。
#
# (作者注) 第11-13行，计算日频收益率和动量因子（UMD）。
#
# (作者注) 第14-16行，读取了pb和市值数据用来计算Fama-French三因子模型中的市场收益率、SMB因子收益率和HML因子收益率。
#
# (作者注) 第17-20行，调用cal\_premium方法计算因子溢价（因子收益率）。
#
# (作者注) 第21-25行，用BaseDataLoader类将因子溢价的dataframe转换为多个数据透视表。
#
# (作者注) 第26-29行，调用cal\_beta计算最后的因子。
...
# (作者注) 第2行，确定当前计算的时间戳。
#
# (作者注) 第3-6行，拼接数据计算行业中涨跌停标的的比例。
#
# (作者注) 第6-10行，计算UMD因子的因子溢价和市场收益率。
#
# (作者注) 第11-14行，计算SMB和HML因子的因子溢价。
#
# (作者注) 第15行，将series转换为dataframe。
#
# (作者注) 第16-21行，插入计算的结果。
...
# (作者注) SMB和HML因子溢价的计算和UMD的其实是类似的，唯一需要理解的就是Fama-French的原理。当然，笔者的实现和最初的Fama-French不同，笔者是按照全市场每日的pb和市值来计算，Fama-French是按照一个固定的时间（一年一次）重新计算分组然后，并且标的池可以是沪深300。
...
# (作者注) 最后就是时序回归部分了，这部分最近介绍了很多次了，也不赘述了。
